...
Numbers can be converted to characters using `as.character()` and all numbers will be converted to `TRUE` unless it is equal to 0 when we use the `as.logical()` function.
...
Let's now try to convert the character object to a numeric and logical type
...
```{r}
# use as.character() funciton to convert logical_var to a character type
