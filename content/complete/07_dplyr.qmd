...
1. Use the pipe to remove the continent column. Save the output as an object called `gapminder_tmp`.
...
Use `count()` to compute the *median* lifeExp for each country, and then arrange the countries in descending order of maximum `pop` value.
